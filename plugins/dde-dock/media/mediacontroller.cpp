...
// CanGoNext/CanGoPrevious 是 MPRIS Player 接口的必备属性，但个别播放器没有实现，
// 读取结果是无效值。这种情况按“可以切换”处理，否则卡片上的切换按钮会一直置灰、
// 完全没法切歌；快捷面板里的切歌按钮也是始终可点的。
bool trackSwitchAllowed(QDBusAbstractInterface *player, const char *property)
{
    const QVariant value = player->property(property);
...
void MediaController::finishLoadMediaPath(const QString &path)
{
    ...
    // Start with the service name, the nicer "Identity" arrives asynchronously.
    m_appName = playerIdentity(path);
    fetchPlayerIdentity(path);
    ...
    connect(m_mediaPlayer2, &DBusMediaPlayer2::CanGoNextChanged, this, [this] {
        Q_EMIT canGoNextChanged(canGoNext());
    });
    connect(m_mediaPlayer2, &DBusMediaPlayer2::CanGoPreviousChanged, this, [this] {
...
void MediaController::fetchPlayerIdentity(const QString &path)
{
    // Asked asynchronously and cached in m_appName: this ends up on the hover
    // path of the card tooltip, where a blocking call would stall the plugin.
    QDBusInterface propertiesInter(path,
                                   QStringLiteral("/org/mpris/MediaPlayer2"),
                                   QStringLiteral("org.freedesktop.DBus.Properties"),
                                   QDBusConnection::sessionBus());
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this,
        ...
        // The active player may have changed while the call was in flight, keep
        // the name that belongs to it instead of the one we asked for.
        if (path != m_path) {
            return;
        }
        ...
        const QString identity = reply.value().variant().toString();
        if (identity.isEmpty() || identity == m_appName) {
            return;
        }
...
void MediaController::onMetaDataChanged()
{
    ...
    // 切歌后能否继续上一首/下一首会变化，而部分播放器只上报 Metadata，
    // 不会单独上报 CanGoNext/CanGoPrevious，这里主动刷新一次，
    // 避免按钮的置灰状态停留在上一首歌的状态上。
    syncPlayerProperties();
}
...
{
    return m_mediaPlayer2 && trackSwitchAllowed(m_mediaPlayer2, "CanGoNext");
}

bool MediaController::canGoPrevious() const
{
    return m_mediaPlayer2 && trackSwitchAllowed(m_mediaPlayer2, "CanGoPrevious");
}
...
QString MediaController::appName() const
{
    return m_appName;
}
...
QString MediaController::artSource() const
{
    ...
    const QString url = m_mediaPlayer2->metadata().value("mpris:artUrl").toString();
    const QUrl parsedUrl(url);

    // 安全校验：仅允许本地文件协议，防止通过 QML Image 组件发起 SSRF 攻击
